...
This page serves as the contribution guide for the `DeepART` package.
...
The main point of contact is the [GitHub issues](https://github.com/AP6YC/DeepART/issues) page for the project.
...
```sh
git clone git@github.com:AP6YC/DeepART.jl.git
julia --project=.
```
...
The `DeepART` package follows the [GitFlow](https://nvie.com/posts/a-successful-git-branching-model/) git working model.
...
3. When you are satisfied with your changes, initiate a [GitHub pull request](https://github.com/AP6YC/DeepART.jl/pulls) (PR) to merge the feature branch with `develop`.
...
The `DeepART` project has the following file structure:
...
If you simply have suggestions for improvement, Sasha Petrenko (<petrenkos@mst.edu>) is the current developer and maintainer of the `DeepART` package, so please feel free to reach out with thoughts and questions.
